...
const SubmissionDetail = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const found = list
                    .filter(s => s.studentId?._id === studentId)
    ...
    if (submission.aiScore === null) {
        alert("Chưa có điểm AI!");
        return;
    }

    setGradeInput(submission.aiScore);
    setFeedbackInput(submission.aiFeedback || "");
    };
    const getImageUrl = (path) => { if (!path) return ""; return path.startsWith("http") ? path : `http://localhost:5000/${path.replace(/^\/+/, "")}` }
